...
void W25qxx_SpiTransmit(uint8_t *buffer, uint32_t size) {
    uint32_t tx_bytes = 0;

    while (tx_bytes < size) {
        if (LL_SPI_IsActiveFlag_TXE(_W25QXX_SPI)) {
            LL_SPI_TransmitData8(_W25QXX_SPI, *(buffer + tx_bytes));
            tx_bytes++;
        }
    }
    while (LL_SPI_IsActiveFlag_BSY(_W25QXX_SPI)) {
    }
}

void W25qxx_SpiReceive(uint8_t *buffer, uint32_t size) {
    uint32_t rx_bytes = 0;
    uint32_t tx_bytes = 0;

    /* Flush RX FIFO */
    while (LL_SPI_IsActiveFlag_RXNE(_W25QXX_SPI)) {
       LL_SPI_ReceiveData8(_W25QXX_SPI);
    }

    while (rx_bytes < size) {
        if (tx_bytes < size && LL_SPI_IsActiveFlag_TXE(_W25QXX_SPI)) {
            LL_SPI_TransmitData8(_W25QXX_SPI, W25QXX_DUMMY_BYTE);
            tx_bytes++;
        }
        if (LL_SPI_IsActiveFlag_RXNE(_W25QXX_SPI)) {
            *(buffer + rx_bytes) = LL_SPI_ReceiveData8(_W25QXX_SPI);
            rx_bytes++;
        }
    }
}
...
uint8_t W25qxx_Spi(uint8_t Data) {
    ...
    LL_SPI_TransmitData8(_W25QXX_SPI, Data);
    while (!LL_SPI_IsActiveFlag_RXNE(_W25QXX_SPI)) {
    }
    return LL_SPI_ReceiveData8(_W25QXX_SPI);
}
#else
void W25qxx_SpiTransmit(uint8_t *buffer, size_t size) {
    HAL_SPI_Transmit(&_W25QXX_SPI, buffer, size, 100);
}
void W25qxx_SpiReceive(uint8_t *buffer, size_t size) {
    HAL_SPI_Receive(&_W25QXX_SPI, buffer, size, 2000);
}
uint8_t W25qxx_Spi(uint8_t  Data)
{
    uint8_t ret;
...
bool W25qxx_Init(void) {
    w25qxx.Lock = 1;
    W25qxx_Delay(100); //while(HAL_GetTick()<100) W25qxx_Delay(1);
    W25qxx_Set_CS();
    W25qxx_Delay(100);
    uint32_t id;
#if (_W25QXX_DEBUG==1)
...
bool W25qxx_IsEmptyPage(uint32_t Page_Address, uint32_t OffsetInByte,
    ...
    for (i = OffsetInByte; i < w25qxx.PageSize; i += sizeof(pBuffer)) {
        ...
        W25qxx_Spi(0);
        W25qxx_SpiReceive(pBuffer, sizeof(pBuffer));
        W25qxx_Set_CS();
...
void W25qxx_WritePage(uint8_t *pBuffer, uint32_t Page_Address,
    ...
    W25qxx_SpiTransmit(pBuffer, NumByteToWrite_up_to_PageSize);
    W25qxx_Set_CS();
    W25qxx_WaitForWriteEnd();
#if (_W25QXX_DEBUG==1)
...
void W25qxx_ReadBytes(uint8_t *pBuffer, uint32_t ReadAddr,
    ...
    W25qxx_Spi(0);
    W25qxx_SpiReceive(pBuffer, NumByteToRead);
    W25qxx_Set_CS();
#if (_W25QXX_DEBUG==1)
	StartTime = HAL_GetTick()-StartTime; 
...
void W25qxx_ReadPage(uint8_t *pBuffer, uint32_t Page_Address,
    ...
    W25qxx_Spi(Page_Address & 0xFF);
    W25qxx_Spi(0);
    W25qxx_SpiReceive(pBuffer, NumByteToRead_up_to_PageSize);
    W25qxx_Set_CS();
#if (_W25QXX_DEBUG==1)
	StartTime = HAL_GetTick()-StartTime; 
